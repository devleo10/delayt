...
export async function markRunCancelled(id: string): Promise<void> {
  const pool = await getPoolReady();
  await pool.query(`UPDATE test_runs SET cancelled = true WHERE id = $1`, [id]);
}

export async function isRunCancelled(id: string): Promise<boolean> {
  const pool = await getPoolReady();
  const result = await pool.query(`SELECT cancelled FROM test_runs WHERE id = $1`, [id]);
  return result.rows[0]?.cancelled === true;
}

export async function importCompletedRun(
  id: string,
  slug: string,
  endpoints: object[],
  requestCount: number,
  ...
  const pool = await getPoolReady();
  await pool.query(
    `INSERT INTO test_runs (id, slug, endpoints, request_count, status, started_at, completed_at)
     VALUES ($1, $2, $3, $4, 'completed', NOW(), NOW())`,
    [id, slug, JSON.stringify(endpoints), requestCount]
  );

  for (const row of requests) {
    await insertRequestResult({ ...row, run_id: id });
  }
}