...
import { AnalyticsResult, RawRequestData, formatLatency } from "@delayt/shared";
...
const ResultsTable: React.FC<ResultsTableProps> = ({ results, rawData = [] }) => {
  ...
  const copyAsMarkdown = async () => {
    setCopyError(null);
    const header =
      "| Endpoint | Method | p50 | p95 | p99 | Success Rate | Requests |";
    const separator =
      "|----------|--------|-----|-----|-----|--------------|----------|";
  ...
  const downloadAsJSON = () => {
    ...
    const exportFileDefaultName = `delayt-results-${new Date().getTime()}.json`;
  ...
  const downloadAsCSV = () => {
    ...
    link.setAttribute("download", `delayt-results-${new Date().getTime()}.csv`);
    link.click();
  };

  const downloadRawAsJSON = () => {
    ...
    linkElement.setAttribute(
      "download",
      `delayt-raw-${new Date().getTime()}.json`,
    );
    linkElement.click();
  };

  const downloadRawAsCSV = () => {
    const headers = [
      "endpoint",
      "method",
      "latency_ms",
    ...
    const link = document.createElement("a");
    link.setAttribute("href", url);
    link.setAttribute("download", `delayt-raw-${new Date().getTime()}.csv`);
    link.click();
  ...
  return (
    <div className="results-table-container">
      <table
        ...
        role="table"
        aria-label="API latency test results"
      >
        <caption className="sr-only">
          Performance metrics for {results.length} API endpoints showing latency
        ...
        <thead>
          <tr>
            ...
            <th scope="col">
              <Tooltip
                content="50% of requests are this fast or faster. Your median experience."
                position="bottom"
              >
                <span
                  style={{ borderBottom: "1px dotted #9ca3af", cursor: "help" }}
                >
                  p50
                </span>
              </Tooltip>
            ...
            <th scope="col">
              <Tooltip
                content="95% of requests are this fast or faster. Most important metric. Where your users live."
                position="bottom"
              >
                <span
                  style={{ borderBottom: "1px dotted #9ca3af", cursor: "help" }}
            ...
            <th scope="col">
              <Tooltip
                content="99% of requests are this fast or faster. Worst-case scenario. Only 1% of users wait longer."
                position="bottom"
              >
                <span
                  style={{ borderBottom: "1px dotted #9ca3af", cursor: "help" }}
          ...
          {results.map((result, index) => {
            ...
            return (
              ...
              >
                ...
                <td
                  className={`latency-cell ${getLatencyClass(result.p50)}`}
                  role="gridcell"
                >
                  <span
                    aria-label={`p50 latency: ${formatLatency(result.p50)}`}
                  >
                    {formatLatency(result.p50)}
                  </span>
                </td>
                <td
                  className={`latency-cell p95-cell ${getLatencyClass(result.p95)}`}
                  role="gridcell"
                >
                  <span
                    aria-label={`p95 latency: ${formatLatency(result.p95)}`}
                  >
                    {formatLatency(result.p95)}
                  </span>
                </td>
                <td
                  className={`latency-cell ${getLatencyClass(result.p99)}`}
                  role="gridcell"
                >
                  <span
                    aria-label={`p99 latency: ${formatLatency(result.p99)}`}
                  >
                ...
                <td>
                  <span
                    className={`success-badge ${getSuccessClass(successRate)}`}
                  >
      ...
      <div className="table-footer">
        ...
        <div>
          ...
          <button className="download-button" onClick={downloadAsJSON}>
            JSON
          </button>
          ...
          {hasRawData && (
            <>
              <button className="download-button" onClick={downloadRawAsJSON}>
                Raw JSON
              </button>
              <button className="download-button" onClick={downloadRawAsCSV}>
                Raw CSV
              </button>
            </>
          )}
