...
npm run dev`}</code></pre>
            <p>
              Open <strong>http://localhost:3000</strong> for the app.
            </p>
            <p>
              Optional: copy <code>app/.env.example</code> to <code>app/.env</code> if you
          ...
          <section className="docs-section" id="web-app">
            ...
            <button type="button" className="docs-inline-cta" onClick={() => router.push('/app')}>
              Open the app
            </button>
          </section>
  ...
  -n 50`}</code></pre>
          ...
          <section className="docs-section" id="cli">
            <h2>CLI for CI/CD</h2>
            <p>
              The CLI runs without a database. Use it for 50–200 requests, auth headers, and CI
              gates. The web app caps at 20 requests per endpoint.
            </p>
...
# Maximum per run
npx @delayt/cli run -u https://api.example.com/health -n 200`}</code></pre>
            <p>
              With fewer than ~30 requests, p95/p99 swing easily from one slow response. If asserts
              fail on a small <code>-n</code>, retry with <code>-n 50</code> before changing
              thresholds.
            </p>

            <h3 id="cli-auth">Auth &amp; headers</h3>
            <p>
              Pass headers with repeated <code>-H</code>. Same flags work for private APIs and CI
              secrets.
            </p>
...
npx @delayt/cli run \\
  ...
  -n 50 \\
  --assert-p95=500 \\
  --assert-p99=1000`}</code></pre>
            <p>
              <code>--assert-p50</code>, <code>--assert-p95</code>, <code>--assert-p99</code> are
              in milliseconds. Omit them to only measure without failing.
            </p>
...
delayt run -u https://api.example.com/health -n 50`}</code></pre>
            <p>
              Exit codes: <code>0</code> pass · <code>1</code> assertion failed · <code>2</code>{' '}
          ...
          <section className="docs-section" id="tips">
            ...
            <ul>
              <li>
                Web runs use up to <strong>20 requests</strong> for a quick smoke test. Use the CLI
                with <strong>30–50 requests</strong> per endpoint for stable percentile estimates.
              </li>
          ...
          <footer className="docs-footer">
            ...
            <div className="docs-footer-links">
              ...
              <button type="button" className="docs-footer-link" onClick={() => router.push('/app')}>
                App
              </button>
              <a
